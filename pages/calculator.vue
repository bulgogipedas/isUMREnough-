 ...
 */
import { ArrowLeft, Sun, Moon, Globe } from 'lucide-vue-next'
...
<template>
  <div class="min-h-screen bg-slate-100 dark:bg-slate-950 transition-colors duration-500">
    ...
    <div class="fixed inset-0 bg-gradient-to-br from-slate-50 via-slate-100 to-amber-50/30 dark:from-slate-950 dark:via-slate-900 dark:to-amber-950/20 pointer-events-none transition-colors duration-500" />

    <!-- Minimal Header -->
    <header class="relative z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-800 px-4 py-3 sticky top-0 transition-colors duration-300">
      <div class="max-w-7xl mx-auto flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <NuxtLink to="/" class="flex items-center gap-2.5">
            <div class="w-10 h-10 bg-gradient-to-br from-amber-400 to-yellow-500 rounded-2xl flex items-center justify-center shadow-lg shadow-amber-200/50 dark:shadow-amber-900/30">
              <span class="text-lg">💰</span>
            </div>
            <div class="hidden sm:block">
              <h1 class="font-bold text-slate-800 dark:text-white text-sm tracking-tight">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="px-3 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all duration-300"
          >
            <span class="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase">{{ locale }}</span>
          </button>

          <!-- Live Data Badge -->
          <div class="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-100 dark:border-emerald-800 rounded-full">
            <span class="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
            <span class="text-[10px] font-semibold text-emerald-700 dark:text-emerald-400 tracking-wide">LIVE DATA 2024</span>
          </div>
        </div>
      </div>
    </header>
    ...
    <Transition name="fade">
      ...
      >
        <div class="text-center">
          ...
          <p class="text-slate-600 dark:text-slate-400 font-medium">{{ t('map.loading') }}</p>
        </div>
      </div>
    </Transition>
      ...
      <div class="max-w-7xl mx-auto px-4 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-start">
          ...
          <div class="order-1 lg:order-2 lg:col-span-7">
            <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-[2rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-white/50 dark:border-slate-800 p-6 lg:p-8 transition-colors duration-300">
